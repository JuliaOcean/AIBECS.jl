...
function build_T(grid)
    ...
    T = CG.T_diffusion(10.0e6u"m^3/s", 1, 2, v3D, nb)
    # between high-lat and deep (3 and 5)
    T += CG.T_diffusion(53.0e6u"m^3/s", 3, 5, v3D, nb)
    # between low-lat and deep (2 and 4)
    T += CG.T_diffusion(1.0e6u"m^3/s", 2, 4, v3D, nb)
    # (trick) fast mixing between boxes of the same 3-box-model box
    T += CG.T_diffusion(1.0e10u"m^3/s", 1, 3, v3D, nb)
    T += CG.T_diffusion(1.0e10u"m^3/s", 5, 6, v3D, nb)
    T += CG.T_diffusion(1.0e10u"m^3/s", 4, 6, v3D, nb)

    # Overturning circulation
    T += CG.T_advection(19.0e6u"m^3/s", [1, 3, 5, 6, 4, 2, 1], v3D, nb)
