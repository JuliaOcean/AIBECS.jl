...
The ocean is a complex system that transports, mixes, and transforms chemical constituents on a multitude of time and length scales.
A large and growing database of biogeochemical observations are being used to evaluate and constrain marine biogeochemical models to improve our understanding of these chemical transformations.
The simplest of these models consist of only a few boxes, with solutions that are easily obtained by hand or on a simple desktop computer.
In contrast, the most advanced models have high-resolution three-dimensional meshes that require high-performance computing (HPC) clusters and a considerable amount of computational-science expertise.
...
All models are wrong by construction because of the assumptions made to simplify complex real-world mechanisms [@Box_1979].
These assumptions give rise to model parameters that are generally adjusted to minimize the mismatch between simulation and observations.
Once the model of the cycle of a marine tracer is implemented,
it is thus natural to try to improve the skill of the model by optimizing the value of its parameters.
AIBECS.jl was thus also designed with parameter fitting in mind.
Specifically, AIBECS and satellite packages provide tools for generating first and second-order derivatives that can be used to efficiently optimize biogeochemical parameters.
...
Offering a truly open-source solution, the AIBECS.jl framework improves on the AO on a number of other fronts to target teaching as well as exploratory and cutting-edge research.
Simple AIBECS.jl simulations can be produced in minutes in interactive notebooks while resource-hungry projects, e.g., for optimizing models with multiple tracers and nonlinear interactions, can be easily version-controlled, hosted, and run on HPC clusters.
...
The package [documentation](https://juliaocean.github.io/AIBECS.jl/stable/), which is built through continuous integration (CI), includes runnable tutorial examples that are both available online for consultation and as Jupyter notebooks.
...
In spite of reduced resolution and steady-state assumption, "simplified" ocean circulation models such as the OCIM are at the forefront of oceanographic research, as evidenced by their increasing use in recent literature.
...
Although they relied on simpler models than traditional Ocean General Circulation Models (OGCMs), most of these studies required significant expertise and were built using private implementations that remain difficult to reproduce.
...
It provides a free, open-source, unified framework for biogeochemical-tracer-modelling studies that use steady-state circulation models.
...
Finally, bridging packages could be implemented for improved composability with, e.g., statistical ([Turing.jl](https://github.com/TuringLang/Turing.jl) [@ge2018t]), optimization, and plotting software ([Makie.jl](https://github.com/JuliaPlots/Makie.jl) [@Makie.jl; @Danisch_Krumbiegel_2021]) to meet demand.
