...
function plot(results::SteadyStateSolution; 
    ...
    fig = Makie.Figure()
    axi = Makie.Axis(fig[1, 1])
    hm=plot!(results; lon = lon, lat = lat, depth = depth, kwargs...)
...
begin
    fig = Makie.Figure();
    Makie.Axis(fig[1, 1]);
    plot!(results,depth=1000,colorrange=(0,5e10))
    Makie.Axis(fig[1, 2]);
    plot!(results,depth=2000,colorrange=(0,5e10))
    fig
end
```
"""
function plot!(results::SteadyStateSolution; 
    ...
    ax=get_axis(grd)
    axi=Makie.current_axis()

    if !isnothing(depth)
        slice=AIBECS.horizontalslice(state, grd; depth = depth);
        axi.title = "state at $(depth) m"
        axi.xticks = ax.lonticks
        axi.yticks = ax.latticks
        hm=Makie.heatmap!(axi, ax.lon, ax.lat, slice';kwargs...)
    elseif !isnothing(lon)
        ...
        hm=Makie.heatmap!(axi, ax.lat, ax.depth, mer',kwargs...)
    elseif !isnothing(lat)
        ...
        hm=Makie.heatmap!(axi, ax.lon, ax.depth, mer',kwargs...)
    else
